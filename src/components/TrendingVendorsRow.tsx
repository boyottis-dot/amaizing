        ...
        {vendors.map((v) => (
          ...
          >
            ...
            <div className="absolute top-3 left-3 right-3 z-10">
              <p className="text-white text-base font-bold truncate drop-shadow-md">{v.name}</p>
              <p className="text-white/60 text-[10px] font-medium">{v.followers} followers</p>
            </div>
            <div className="absolute inset-0 flex items-center justify-center z-10">
              <img src={v.img} alt={v.name} className="w-[90px] h-[90px] rounded-full object-cover border-[3px] border-white/30 shadow-xl" />
            </div>

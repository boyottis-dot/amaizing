...
const StackedCards = () => {
  ...
  const navigate = useCallback(
    (dir: "left" | "right") => {
      ...
      setDirection(dir);
      setTimeout(() => {
        setActiveIndex((prev) => dir === "right" ? (prev + 1) % cards.length : (prev - 1 + cards.length) % cards.length);
        setDirection(null);
  ...
  const getCardStyle = (index: number) => {
    ...
    if (offset === total - 1) return { zIndex: total - 1, transform: "translateX(-28px) scale(0.95)", opacity: 1, filter: "brightness(0.45)" };
    return { zIndex: 0, transform: `translateX(${offset > total / 2 ? -48 : 48}px) scale(0.9)`, opacity: 0, filter: "brightness(0.3)" };
  };
      ...
      <div className="flex flex-col items-center">
        <div
          ...
          {cards.map((card, i) => {
            ...
            return (
              <div key={card.id} className="absolute inset-0 overflow-hidden rounded-[20px] transition-all duration-500" style={{ ...style, transitionTimingFunction: "cubic-bezier(0.16, 1, 0.3, 1)" }}>
        ...
        <div className="mt-4 flex items-center gap-4">
          <button onClick={() => navigate("left")} className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 backdrop-blur-xl border border-white/20 active:scale-90 transition-all duration-200" aria-label="Previous">

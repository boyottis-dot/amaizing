      ...
      <div className="min-h-screen bg-background max-w-md mx-auto relative flex flex-col">
        ...
        <header className="sticky top-0 z-50 flex items-center gap-3 px-4 py-3 bg-background/80 backdrop-blur-xl border-b border-border/20">
          <button
            ...
            className="flex items-center justify-center h-9 w-9 rounded-full bg-background/60 backdrop-blur-xl shadow-[0_2px_12px_-2px_hsl(var(--foreground)/0.08)] border border-border/30"
          >
              ...
              <div
                className={`max-w-[75%] rounded-[18px] px-4 py-2.5 ${
                  msg.fromMe
                    ...
                    : "bg-secondary/80 text-foreground border border-border/30 rounded-bl-[6px]"
                }`}
              >
                <p className="text-[13px] leading-relaxed">{msg.text}</p>
                <p className={`text-[10px] mt-1 ${msg.fromMe ? "text-background/50" : "text-muted-foreground"}`}>
                  {msg.time}
                </p>
              </div>
            </div>
    ...
    <div className="min-h-screen bg-background max-w-md mx-auto relative">
      ...
      <header className="sticky top-0 z-50 flex items-center gap-3 px-4 py-4 bg-background/80 backdrop-blur-xl">
        ...
        <div className="flex-1 flex justify-center">
          <div className="rounded-full bg-background/50 backdrop-blur-xl border border-border/30 shadow-[0_2px_12px_-2px_hsl(var(--foreground)/0.08)] px-4 py-1.5">
        ...
        {contacts
          ...
          .map((chat) => (
            <button
              ...
              className="flex w-full items-center gap-3 rounded-[18px] bg-secondary/40 backdrop-blur-xl px-4 py-3.5 text-left transition-colors hover:bg-secondary/60 border border-border/20"
            >
              <div className="relative shrink-0">
                <img
                  src={chat.avatar}
                  alt={chat.name}
                  className="h-12 w-12 rounded-full object-cover ring-2 ring-border/20"
                />
                {chat.online && (

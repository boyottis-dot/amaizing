    ...
    <div className="min-h-screen bg-background max-w-md mx-auto relative">
      ...
      <header className="sticky top-0 z-50 flex items-center gap-3 px-4 py-4 bg-background/80 backdrop-blur-xl">
        ...
        <div className="flex-1 flex justify-center">
          <div className="rounded-full bg-background/50 backdrop-blur-xl border border-border/30 shadow-[0_2px_12px_-2px_hsl(var(--foreground)/0.08)] px-5 py-2">
            <span className="text-sm font-bold text-foreground">Cart · {items.length}</span>
          </div>
        </div>
        <div className="w-9" />
      </header>
        ...
        <AnimatePresence mode="popLayout">
          {items.map((item) => (
            ...
            >
              {/* Product image */}
              <div
                className="aspect-square rounded-2xl overflow-hidden shrink-0 w-24 h-24"
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <div className="flex items-center rounded-full bg-foreground overflow-hidden">
                    <button
                      ...
                      className="w-9 h-8 flex items-center justify-center text-background"
                    >
                      <Plus size={14} />
                    </button>
                    <button
      ...
      <div className="fixed bottom-6 inset-x-0 z-30 flex justify-center px-4">
        <div className="w-full max-w-[calc(28rem-2rem)] rounded-3xl bg-secondary/70 backdrop-blur-xl border border-border/30 shadow-[0_-8px_40px_-8px_hsl(var(--foreground)/0.12)] p-5">
          ...
          >
            <span className="text-[15px] font-bold text-background tracking-[-0.01em]">
              Proceed to Checkout
            </span>
